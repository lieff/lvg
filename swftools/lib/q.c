...
heap_t* heap_clone(heap_t*o)
{
    ...
    h->elements = malloc(sizeof(void*)*h->size);
    int t;
    for(t=0;t<h->size;t++) {
        h->elements[t] = malloc(h->elem_size);
...
void heap_put(heap_t*h, void*e)
{
    ...
    if(pos>=h->max_size) {
        ...
        h->elements = (void**)realloc(h->elements, h->max_size*sizeof(void*));
...
string_t*string_dup3(string_t*o)
{
    ...
    string_t*s = malloc(sizeof(string_t)+o->len+1);
    s->len = o->len;
    s->str = (const char*)(s+1);
