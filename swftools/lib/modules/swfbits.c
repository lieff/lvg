...
static void file_init_destination(j_compress_ptr cinfo)
{
    ...
    fmgr->buffer = (JOCTET*)malloc(OUTBUFFER_SIZE);
    if(!fmgr->buffer) {
        perror("malloc");
...
int RFXSWF_deflate_wraper(TAG * t, z_stream * zs, int finish)
{
    U8 *data = (U8*)malloc(OUTBUFFER_SIZE);
    zs->next_out = data;
