...
//    nvgFillColor(vg, nvgRGBA(0,0,0,128));
    ...
    headerPaint = nvgLinearGradientTCC(vg, x,y,x,y+15, 255,255,255,8, 0,0,0,16);
    nvgBeginPath(vg);
...
void drawDropDown(NVGcontext* vg, const char* text, float x, float y, float w, float h)
{
    ...
    bg = nvgLinearGradientTCC(vg, x,y,x,y+h, 255,255,255,16, 0,0,0,16);
    nvgBeginPath(vg);
...
void drawButton(NVGcontext* vg, int preicon, const char* text, float x, float y, float w, float h, NVGcolor col)
{
    ...
    bg = nvgLinearGradientTCC(vg, x,y,x,y+h, 255,255,255,isBlack(col)?16:32, 0,0,0,isBlack(col)?16:32);
    nvgBeginPath(vg);
    nvgRoundedRect(vg, x+1,y+1, w-2,h-2, cornerRadius-1);
    if (!isBlack(col)) {
        nvgFillColor(vg, col);
...
//    nvgClearState(vg);
    ...
    knob = nvgLinearGradientTCC(vg, x,cy-kr,x,cy+kr, 255,255,255,16, 0,0,0,16);
    nvgBeginPath(vg);
...
void drawEyes(NVGcontext* vg, float x, float y, float w, float h, float mx, float my, float t)
{
    ...
    NVGcolor c1 = nvgRGBA(0,0,0,32);
    NVGcolor c2 = nvgRGBA(0,0,0,16);
    //printf("nvgLinearGradient(vg, %f,%f, %f,%f, nvgRGBA(%f,%f,%f,%f), nvgRGBA(%f,%f,%f,%f))\n",
    //  x,y+h*0.5f,x+w*0.1f,y+h, c1.r, c1.g, c1.b, c1.a, c2.r, c2.g, c2.b, c2.a); fflush(stdout);
    //bg = nvgLinearGradient(vg, x,y+h*0.5f,x+w*0.1f,y+h, nvgRGBA(0,0,0,32), nvgRGBA(0,0,0,16));
    bg = nvgLinearGradientTCC(vg, x,y+h*0.5f,x+w*0.1f,y+h, 0,0,0,32, 0,0,0,16);
    nvgBeginPath(vg);
    ...
    bg = nvgLinearGradientTCC(vg, x,y+h*0.25f,x+w*0.1f,y+h, 220,220,220,255, 128,128,128,255);
    nvgBeginPath(vg);
...
void drawGraph(NVGcontext* vg, float x, float y, float w, float h, float t)
{
    ...
    bg = nvgLinearGradientTCC(vg, x,y,x,y+h, 0,160,192,0, 0,160,192,64);
    nvgBeginPath(vg);
...
void drawSpinner(NVGcontext* vg, float cx, float cy, float r, float t)
{
    ...
    paint = nvgLinearGradientTCC(vg, ax,ay, bx,by, 0,0,0,0, 0,0,0,128);
...
//    nvgClearState(vg);
    ...
    fadePaint = nvgLinearGradientTCC(vg, x,y,x,y+6, 200,200,200,255, 200,200,200,0);
    nvgBeginPath(vg);
    ...
    fadePaint = nvgLinearGradientTCC(vg, x,y+h,x,y+h-6, 200,200,200,255, 200,200,200,0);
    nvgBeginPath(vg);
...
/*    nvgBeginPath(vg);
    ...
    for (i = 0; i < 6; i++) {
        ...
        paint = nvgLinearGradientTCC(vg, ax,ay, bx,by, a0/(NVG_PI*2),1.0f,0.55f,255, a1/(NVG_PI*2),1.0f,0.55f,255);
        nvgFillPaint(vg, paint);
        nvgFill(vg);
    }
    ...
    paint = nvgLinearGradientTCC(vg, r,0, ax,ay, hue,1.0f,0.5f,255, 255,255,255,255);
    nvgFillPaint(vg, paint);
    nvgFill(vg);
    //paint = nvgLinearGradient(vg, (r+ax)*0.5f,(0+ay)*0.5f, bx,by, nvgRGBA(0,0,0,0), nvgRGBA(0,0,0,255));
    paint = nvgLinearGradientTCC(vg, (r+ax)*0.5f,(0+ay)*0.5f, bx,by, 0,0,0,0, 0,0,0,255);
    nvgFillPaint(vg, paint);
    nvgFill(vg);
    nvgStrokeColor(vg, nvgRGBA(0,0,0,64));
    nvgStroke(vg);
...
int loadDemoData(NVGcontext* vg, DemoData* data)
{
    ...
    for (i = 0; i < 12; i++) {
        ...
        buf = lvgGetFileContents(file, &size);
        data->images[i] = nvgCreateImageMem(vg, 0, buf, size);
        lvgFree(buf);
        if (data->images[i] == 0) {
    ...
    buf = lvgGetFileContents("entypo.ttf", &size);
    data->fontIcons = nvgCreateFontMem(vg, "icons", buf, size, 1);
    if (data->fontIcons == -1) {
        printf("Could not add font icons.\n");
        return -1;
    }
    buf = lvgGetFileContents("Roboto-Regular.ttf", &size);
    data->fontNormal = nvgCreateFontMem(vg, "sans", buf, size, 1);
    if (data->fontNormal == -1) {
        printf("Could not add font italic.\n");
        return -1;
    }
    buf = lvgGetFileContents("Roboto-Bold.ttf", &size);
    data->fontBold = nvgCreateFontMem(vg, "sans-bold", buf, size, 1);
    if (data->fontBold == -1) {
